...
from suma_health.healthcare.doctype.fee_validity.fee_validity import (
	check_fee_validity,
	get_fee_validity,
	manage_fee_validity,
)
...
from suma_health.healthcare.utils import get_appointment_billing_item_and_rate
...
class PatientAppointment(Document):
	...
	def validate_service_unit(self):
		if self.inpatient_record and self.service_unit:
			from suma_health.healthcare.doctype.inpatient_medication_entry.inpatient_medication_entry import (
	...
	def set_payment_details(self):
		if frappe.db.get_single_value("Healthcare Settings", "show_payment_popup"):
			...
			# Handle both dictionary and tuple returns (for backward compatibility)
			if isinstance(details, dict):
				service_item = details.get("service_item")
				practitioner_charge = details.get("practitioner_charge")
			elif isinstance(details, tuple) and len(details) >= 2:
				service_item, practitioner_charge = details[0], details[1]
			else:
				service_item, practitioner_charge = None, None
			
			if service_item:
				self.db_set("billing_item", service_item)
			if not self.paid_amount and practitioner_charge:
				self.db_set("paid_amount", practitioner_charge)
